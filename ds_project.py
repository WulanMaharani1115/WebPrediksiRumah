...
# input the numbers
sqft_liv = st.slider("Berapa luas ruang tengah yang anda cari?", int(
    data.sqft_living.min()), int(data.sqft_living.max()), int(data.sqft_living.mean()))
bath = st.slider("Berapa banyak kamar mandi yang dibutuhkan?", int(
    data.bathrooms.min()), int(data.bathrooms.max()), int(data.bathrooms.mean()))
bed = st.slider("Berapa banyak kamar tidur yang dibutuhkan?", int(
    data.bedrooms.min()), int(data.bedrooms.max()), int(data.bedrooms.mean()))
floor = st.slider("Berapa tingkat rumah yang anda inginkan?", int(
    data.floors.min()), int(data.floors.max()), int(data.floors.mean()))
...
if st.button("Cek Harga Rumah?"):
    st.header("Harga rumah berdasarkan kebutuhan anda adalah USD {}".format(
        int(predictions)))
    st.subheader("Range perkiraan harga rumah anda adalah USD {} - USD {}".format(
